...
| 报错 `租约已提交(index=N)但本副本未在 …ms 内追平` | 本副本**没学到**该 commit(操作其实已提交,可安全重试) | 查该副本日志的 `本副本追平租约 index=… 耗时 …ms` 与 `投递耗时 …ms` WARN:多为该进程 worker 被占住/CPU 抢占。**升级到含发现 23 修复的版本**后不应再出现;若仍出现,把这两行 WARN + `/internal/status` 抓下来 |
