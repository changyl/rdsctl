...
pub async fn serve(port: u16) -> io::Result<()> {
    ...
    let token = token_opt();
    tracing::info!(
        "rdsctl agent listening on 0.0.0.0:{port} (token: {})",
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(e) = handle(&mut stream, token.as_deref()).await {
...
async fn handle(stream: &mut TcpStream, token: Option<&str>) -> io::Result<()> {
    ...
    let h_token = lines
        .find(|l| l.to_ascii_lowercase().starts_with(HEADER_TOKEN))
        .map(|l| l[HEADER_TOKEN.len() + 1..].trim().to_string());
    ...
    if method == "GET" && path == "/agent/ping" {
        ...
        return reply(stream, 200, &json!({ "ok": true, "host": host }).to_string()).await;
    }
    if method != "POST" {
...
impl Agent {
    ...
    pub async fn run(&self, container: &str, args: &[&str]) -> Result<(), String> {
        let args: Vec<Value> = args.iter().map(|a| json!(a)).collect();
        let v = self.post("/agent/run", &json!({ "container": container, "args": args })).await?;
        if v["ok"].as_bool() == Some(true) {
            Ok(())
        } else {
    ...
    pub async fn rm(&self, container: &str) -> Result<(), String> {
        let v = self.post("/agent/rm", &json!({ "container": container })).await?;
        if v["ok"].as_bool() == Some(true) {
            Ok(())
        } else {
    ...
    pub async fn container_state(&self, container: &str) -> Option<String> {
        let v = self.post("/agent/state", &json!({ "container": container })).await.ok()?;
        if v["ok"].as_bool() == Some(true) {
    ...
    pub async fn exec_mysql_local(&self, container: &str, user: &str, pass: &str, sql: &str) -> Result<String, String> {
        let v = self
            .post("/agent/sql", &json!({ "container": container, "user": user, "pass": pass, "sql": sql }))
            .await?;
        if v["ok"].as_bool() == Some(true) {
            Ok(v["out"].as_str().unwrap_or("").to_string())
        } else {
    ...
    pub async fn exec_in(&self, container: &str, args: &[String]) -> Result<String, String> {
        let args: Vec<Value> = args.iter().map(|a| json!(a)).collect();
        let v = self.post("/agent/exec", &json!({ "container": container, "args": args })).await?;
        if v["ok"].as_bool() == Some(true) {
            Ok(v["out"].as_str().unwrap_or("").to_string())
        } else {
    ...
    pub async fn wait_mysql_ready(&self, container: &str, user: &str, pass: &str, timeout_secs: u64) -> Result<(), String> {
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(timeout_secs);
        while std::time::Instant::now() < deadline {
            if self.exec_mysql_local(container, user, pass, "SELECT 1").await.is_ok() {
                return Ok(());
            }
            tokio::time::sleep(std::time::Duration::from_millis(800)).await;
        }
        let logs = self.docker(&["logs", "--tail", "30", container]).await.unwrap_or_default();
        Err(format!("agent 端容器 {container} 内 MySQL 未在 {timeout_secs}s 内就绪\n最近日志:\n{logs}"))
    }
}

/// HTTP POST JSON(极简客户端,解析状态码 + Content-Length)
async fn post_json(base: &str, token: Option<&str>, path: &str, payload: &Value) -> Result<Value, String> {
...
async fn request(base: &str, token: Option<&str>, method: &str, path: &str, payload: Option<&Value>) -> Result<Value, String> {
    ...
    let content_length: usize = head
        ...
        .and_then(|v| v.trim().parse().ok())
        .unwrap_or(0);
    let body_text = text[head_end.min(text.len())..].chars().take(content_length).collect::<String>();
    if status != 200 {
        return Err(format!("agent {base} HTTP {status}: {}", body_text.trim()));
    }
    serde_json::from_str(body_text.trim()).map_err(|e| format!("agent 响应解析失败: {e}: {}", body_text.trim()))
}
...
fn split_base(base: &str) -> Result<(String, u16), String> {
    ...
    let port = p.parse::<u16>().map_err(|_| format!("agent 端口非法: {base}"))?;
    Ok((h.to_string(), port))
}
